...
ED* Drzewo::RotujLL(ED* wezel){
  ...
  if(nowy->wsprownowagi == 1){
    wezel->wsprownowagi = nowy->wsprownowagi = 0;
  }
  else{
...
ED* Drzewo::RotujRL(ED* wezel){
  ...
  ED* ojciec = wezel->ojciec;

  pomocniczy->lewy = nowy->prawy;
  if(pomocniczy->lewy) pomocniczy->lewy->ojciec = pomocniczy;
  wezel->prawy = nowy->lewy;
  if(wezel->prawy)
    wezel->prawy->ojciec = wezel;
  nowy->lewy = wezel;
  ...
  wezel->wsprownowagi = (nowy->wsprownowagi == -1) ?  1 : 0;
  pomocniczy->wsprownowagi = (nowy->wsprownowagi ==  1) ? -1 : 0;
  nowy->wsprownowagi = 0;
